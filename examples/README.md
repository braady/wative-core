...
| [20-custom-chain-dialect.test.cjs](20-custom-chain-dialect.test.cjs) | Adding a whole new blockchain — subclass `ChainDialect` and `registerDialect` its vm, the same shape as extending `Provider`. A fictional `xvm` chain becomes a first-class network (and an unregistered vm is still refused), its dialect signs / builds a transaction / builds transfer payloads / derives through the keyless `ChainCtx`, and `XvmSigner` names the per-chain signer just as `EvmSigner` / `SvmSigner` do. |
| [21-token-transfer.test.cjs](21-token-transfer.test.cjs) | `address.buildTransferPayload({ to, asset, amount })` — one intent lowered per chain into the payload for `buildTransaction`: native value, EVM ERC-20 by contract address, SVM SPL by mint (with an idempotent recipient-ATA create), and by token `symbol`. Payload only (does not send); `amount` is raw base units. |
