...
interface NetworkInit {
    ...
    writeEndpoints?: readonly string[];
...
declare class Network {
    ...
    endpointsFor(kind?: "read" | "write" | "default"): readonly string[];
    ...
    static readonly HyperEVM: Network;
    static readonly HyperEVMTestnet: Network;
    static readonly Robinhood: Network;
    static readonly RobinhoodTestnet: Network;
...
declare abstract class Provider {
    ...
    /** Does the STORE already hold an account record under this slug? */
    accountRecordExists(slug: Slug): Promise<boolean>;
    /** Slugs this provider knows are present but could not be opened. */
    unreadableRecordSlugs(): ReadonlyArray<Slug>;
    /** Name stems already occupied in the accounts namespace, lowercased. */
...
interface DecodedTx {
    ...
    readonly svmVersion?: "legacy" | 0;
    readonly feePayer?: string;
    readonly recentBlockhash?: string;
    readonly instructionCount?: number;
    readonly hasAddressLookupTables?: boolean;
...
declare function isRawTx(x: unknown): x is RawTx;

declare function deserializeEvmTx(serialized: string): {
    raw: RawTx;
    decoded: DecodedTx;
};

declare function deserializeSvmTx(serialized: string): {
    raw: RawTx;
    decoded: DecodedTx;
};

declare function deserializeTx(vm: VmToken, serialized: string): {
    raw: RawTx;
    decoded: DecodedTx;
};
...
declare function normalizeTxInput<P extends object>(vm: VmToken, input: P | RawTx | string): NormalizedTx<P>;

type DestinationCode = "EVM_ZERO_ADDRESS" | "EVM_BURN_ADDRESS" | "EVM_PRECOMPILE" | "EVM_TOKEN_SELF_CONTRACT" | "EVM_ERC20_TO_CONTRACT" | "EVM_NATIVE_TO_CONTRACT" | "EVM_DELEGATED_EOA_7702" | "SVM_SOL_TO_OFFCURVE" | "SVM_INCINERATOR" | "SVM_SOL_TO_MINT" | "SVM_SOL_TO_PROGRAM_OR_SYSVAR" | "SVM_SOL_TO_TOKEN_ACCOUNT" | "SVM_SPL_TO_PROGRAM_OWNED" | "SVM_FROZEN_TOKEN_ACCOUNT" | "SVM_NON_TRANSFERABLE" | "RPC_CHECK_UNAVAILABLE" | "SCREENED";

type DestinationSeverity = "refuse" | "confirm" | "warn";

interface DestinationWarning {
    readonly code: DestinationCode;
    readonly chain: "evm" | "svm";
    readonly address: string;
    readonly asset: "native" | "token";
    readonly detail: string;
}

interface DestinationReport {
    readonly action: "allow" | "warn" | "confirm" | "refuse";
    readonly code?: DestinationCode;
    readonly severity?: DestinationSeverity;
    readonly chain: "evm" | "svm";
    readonly address: string;
    readonly asset: "native" | "token";
    readonly warnings: readonly DestinationWarning[];
    readonly rpcChecked: boolean;
}

interface DestinationScreenResult {
    readonly severity: "refuse" | "warn";
    readonly detail: string;
}

type DestinationScreener = (input: {
    readonly chain: "evm" | "svm";
    readonly address: string;
    readonly asset: "native" | "token";
}) => DestinationScreenResult | null | Promise<DestinationScreenResult | null>;

interface DestinationGuardOptions {
    readonly forceUnsafeTransfer?: boolean;
    readonly acknowledgeRiskyDestination?: boolean;
    readonly onWarning?: (warning: DestinationWarning) => void;
    readonly screen?: DestinationScreener;
}

type WativeErrorCode = "WORKSPACE_LOCKED" | "ACCOUNT_LOCKED" | "RECORD_LOCKED" | "BAD_PASSWORD" | "WEAK_PASSWORD" | "RECORD_NOT_FOUND" | "PROVIDER_IO" | "PERMISSION_DENIED" | "DISK_FULL" | "PARAMETER_ERROR"

 | "IRREGULAR_DESTINATION" | "DECRYPT_FAILED" | "ENCRYPT_FAILED" | "ALGORITHM_IRREVERSIBLE" | "INVALID_MNEMONIC" | "INVALID_PRIVATE_KEY" | "UNSUPPORTED_NETWORK" | "TX_BUILD_FAILED" | "TX_SIGN_FAILED" | "TX_SUBMIT_FAILED" | "TX_TIMEOUT" | "TX_DROPPED" | "TX_ABORTED" | "RPC_UNREACHABLE"
...
interface SvmAddressLookupTable {
    readonly address: string;
...
interface SvmTxBuildParams {
    ...
    addressLookupTables?: ReadonlyArray<SvmAddressLookupTable>;
...
interface Transaction {
    ...
    send(opts?: SendOptions): TransactionTracker;
...
type SendRoute = "default" | "staked" | "jito" | "mev";
...
declare class EvmTransaction implements Transaction {
    ...
    set rpcUrl(next: string | undefined);
    ...
    constructor(from: EvmAddress, to: string | null, value: bigint, data: string | undefined, opts: {
        chainId: ChainId;
        type?: EvmTxType;
        nonce?: number;
        gasPrice?: bigint;
...
interface SignRawOptions {
    
    confirm?: (decoded: DecodedTx) => boolean | Promise<boolean>;
    
    acknowledgeBlindSign?: boolean;
    
    allow7702Delegation?: boolean;
...
declare class Address {
    ...
    signIn(params?: SignInParams): SignInResult;
    buildTransaction(params: EvmTxBuildParams): EvmTransaction;
    buildTransaction(params: SvmTxBuildParams): SvmTransaction;
    
    buildTransferPayload(req: TransferRequest, opts?: DestinationGuardOptions): TransferPayload;
    
    checkDestination(req: TransferRequest, opts?: DestinationGuardOptions): Promise<DestinationReport>;
    
    signTransaction<T extends Transaction>(transaction: T): T;
    
    signAll<T extends Transaction>(transactions: readonly T[]): Promise<readonly T[]>;
    
    signRawTransaction(input: RawTx | string, opts?: SignRawOptions): Promise<RawTx>;
    
    sendTransaction(transaction: Transaction, opts?: SendOptions): TransactionTracker;
    
    simulateTransaction(transaction: Transaction): Promise<SimulationResult>;
    
    estimate(input: RawTx | string, opts?: {
        rpcUrl?: string;
...
declare class EvmSigner extends Address {
}
declare class SvmSigner extends Address {
}
...
declare class Wallet {
    ...
    get evm(): EvmSigner | null;
    get svm(): SvmSigner | null;
...
declare class Workspace {
    ...
    get accounts(): AccountCollection;
    get networks(): NetworkCollection;
    
    _noteRpcUrl(url: string): void;
    ...
    _trackedAssetsForSlug(slug: string): ReadonlyArray<{
        symbol: string;
        contractAddress: string | null;
    }>;
}

export { type SinkConfig as $, Account as A, type LogSink as B, type ChainId as C, type DecodedTx as D, type EvmAddress as E, Logger as F, type LoggerConfig as G, type NetworkCollection as H, type NetworkInit as I, type NetworkLike as J, type NetworkRoutes as K, type LogLevel as L, type NormalizedTx as M, Network as N, type OpenOptions as O, Provider as P, Record$1 as Q, type RawTx as R, type RecordInit as S, type RecordType as T, type SendOptions as U, type SendRoute as V, type Settling as W, type SignInParams as X, type SignInResult as Y, type SignRawOptions as Z, type SimulationResult as _, type AccountCollection as a, type Slug as a0, type SvmAddress as a1, type SvmAddressLookupTable as a2, SvmSigner as a3, SvmTransaction as a4, type SvmTxBuildParams as a5, type Transaction as a6, type TransactionReceipt as a7, type TransactionStateChange as a8, type TransactionStatus as a9, type TransactionTracker as aa, type TransferPayload as ab, type TransferRequest as ac, type VmToken as ad, Wallet as ae, type WalletCollection as af, WativeError as ag, type WativeErrorCode as ah, type WativeErrorOpts as ai, Workspace as aj, type WorkspaceConfig as ak, deserializeEvmTx as al, deserializeSvmTx as am, deserializeTx as an, isRawTx as ao, newMnemonic as ap, normalizeTxInput as aq, type SuiAddress as ar, type AccountOrgType as b, Address as c, Asset as d, type AssetBalance as e, type AssetCollection as f, type AssetId as g, type AssetInitObject as h, type AssetRef as i, type ChainVM as j, type Commitment as k, ConsoleSink as l, type ContainerState as m, type DestinationCode as n, type DestinationGuardOptions as o, type DestinationReport as p, type DestinationScreenResult as q, type DestinationScreener as r, type DestinationSeverity as s, type DestinationWarning as t, type EvmRawTx as u, EvmSigner as v, EvmTransaction as w, type EvmTxBuildParams as x, EvmTxType as y, type LogRecord as z };
